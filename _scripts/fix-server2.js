...
async function main() {
  return new Promise((resolve) => {
    conn.on('ready', async () => {
      ...
      try {
        // ====== Step 1: Update docker-compose.yml with health check ======
...
COMPOSEOF`);
        ...
        // ====== Step 2: Update nginx.conf with security hardening ======
        console.log('\n========== STEP 2: Update nginx.conf with security ==========');
...
http {
    ...
    server {
        ...
        # Block suspicious query strings (command injection attempts)
        if (\\$query_string ~* "(union.*select|exec|xp_cmdshell|proc_open|passthru|system\\(|cmd=|bash|xmrig|wget\\s|curl\\s.*\\||chmod)") {
            return 444;
        }
...
`;
        ...
        await execCmd(conn, `cat > /opt/note-prompt/nginx/nginx.conf << 'NGINXEOF'
${nginxConf}
NGINXEOF`);

        // ====== Step 3: Ensure curl is in app image for health check ======
        console.log('\n========== STEP 3: Check if curl exists in app container ==========');
        await execCmd(conn, 'docker exec note-prompt-app which curl 2>&1 || echo "curl not found"');
        
        // ====== Step 4: Check/create health endpoint ======
        console.log('\n========== STEP 4: Verify health endpoint exists ==========');
        await execCmd(conn, 'docker exec note-prompt-app ls /app/.next/server/app/api/health/ 2>&1');

        // ====== Step 5: Reload nginx and recreate containers ======
        ...
        await execCmd(conn, 'docker exec note-prompt-nginx nginx -s reload 2>&1');
        
        // Recreate containers with new compose (this will add health checks)
        ...
        await execCmd(conn, 'curl -s -o /dev/null -w "API: CODE:%{http_code} TIME:%{time_total}s\\n" --max-time 10 https://noteprompt.cn/api/health');
        await execCmd(conn, 'curl -s -o /dev/null -w "LOGIN: CODE:%{http_code} TIME:%{time_total}s\\n" --max-time 10 https://noteprompt.cn/login');
        await execCmd(conn, 'docker ps --format "table {{.Names}}\\t{{.Status}}"');
