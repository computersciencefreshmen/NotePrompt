...
async function main() {
  ...
  // Write fixed nginx config with proper buffer settings and http2 directive
...
NGXEOF
echo "nginx.conf updated"`, '1. Update nginx config with larger buffers');
  ...
  // Test
  await run(conn, 'docker exec note-prompt-nginx nginx -t 2>&1', '3. Nginx config test');

  // Test from server
  await run(conn, 'curl -sk -o /dev/null -w "HTTPS homepage: %{http_code}\\n" https://localhost/ 2>/dev/null', '4. Test HTTPS homepage');
  await run(conn, 'curl -sk -o /dev/null -w "HTTPS health: %{http_code}\\n" https://localhost/api/health 2>/dev/null', '5. Test HTTPS health');
  await run(conn, 'curl -sk -o /dev/null -w "HTTPS login: %{http_code}\\n" https://localhost/login 2>/dev/null', '6. Test HTTPS login');

  // Check nginx error log for any issues
  await run(conn, 'docker logs note-prompt-nginx 2>&1 | tail -10', '7. Nginx logs');
