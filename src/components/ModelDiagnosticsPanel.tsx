...
const diagnosticsCopy = {
  zh: {
    ...
    failed: '模型检测失败',
    title: '模型可用性检测',
    description: '默认只检查配置，不消耗模型额度；实际调用需手动触发低成本探针。',
    checking: '检测中',
    configCheck: '配置检查',
    probe: '低成本调用测试',
    config: '配置',
  ...
  en: {
    ...
    description: 'Configuration checks run by default and do not spend model quota. Use the low-cost probe only when you need a real call.',
    checking: 'Checking',
  ...
  const runDiagnostics = async (nextMode: 'config' | 'probe', force = false) => {
    ...
    try {
      const response = await fetch('/api/v1/ai/diagnostics', {
        ...
        body: JSON.stringify({ mode: nextMode, force }),
      })
